...
void socket_process(void* arg_p){
    ...
    uint32_t cnt = 0;
    while(1){
        int msg_size = recv_message(sock_fd, buffer_p, BUF_SIZE);
        if(msg_size == 0){
            close(sock_fd);
        }
        cnt++;
...
void pthread_work_forever(int serv_fd, uint8_t buffer[BUF_SIZE], uint32_t buffer_size){
    ...
    while(1)
    {
        ...
        if(sock_fd < 0){
            fprintf(stderr, "[SERV][PTHREAD] accept failed with cnt %u\n", socket_cnt);
        }else{
            ...
            fprintf(stderr, "[SERV][PTHREAD] accepted new socket %u\n", socket_cnt);
